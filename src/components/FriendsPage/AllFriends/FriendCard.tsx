...
type TFriendCard = {
    ...
    friendReq?: {
        sent: { friendId: string, _id : string, status: "pending" | "accepted" | "rejected" }[],
        received: { friendId: string, _id : string, status: "pending" | "accepted" | "rejected" }[]
    },
    friends?: string[];
}

const FriendCard: React.FC<TFriendCard> = ({ variant, name, occupation, _id, profilePicture, friendReq, friends }) => {
    ...
    return (
        <div className="bg-primary-70/40 border rounded-xl py-3 px-2 h-[260px] flex flex-col items-center justify-between">
            <div className="size-24 rounded-full">
                <Image src={profilePicture ? profilePicture : ICONS.user} alt="" width={96} height={96} className="size-24 rounded-full object-cover" />
            </div>
            ...
            {variant === "Friend" ? (
                isAlreadyFriend ? (
                    <button disabled className="flex items-center px-4 py-[6px] border border-primary-20 justify-center rounded-3xl w-full text-primary-20 bg-gray-300 cursor-not-allowed">
                        Already Friend
                    </button>
                ) : isRequestSent ? (
                    <button disabled className="flex items-center px-4 py-[6px] border border-primary-20 justify-center rounded-3xl w-full text-primary-20 bg-gray-300 cursor-not-allowed">
                    ...
                    <button onClick={handleSendFriendReq} className="flex items-center px-4 py-[6px] border border-primary-20 justify-center rounded-3xl w-full text-primary-20 hover:bg-primary-20 hover:text-white transition duration-300 font-semibold">
                        {isSendingReq ? (
                            <span>
                                <svg aria-hidden="true" role="status" className="inline w-4 h-4 me-3 text-primary-20 animate-spin" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                    <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="#d1d1d1" />
